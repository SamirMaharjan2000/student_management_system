                    ...
                    <div class="card border-0 shadow">
                        <div class="card-body">
                            ...
                            <form action="{{route('register')}}" method= "post">
                                ...
                                @endif
                                <input type="text" name="email"  class="form-control my-4 py-2" placeholder="E-mail" required/>
                                @if ($errors->has('name'))
                                    <p class="text-danger">{{ $errors->first('email')}}</p>
                                @endif
                                <input type="tel" name="number" class="form-control my-4 py-2" placeholder="Number" required/>
                                <input type="password" name="password" class="form-control my-4 py-2" placeholder="Password" required/>
                                @if ($errors->has('password'))
                                    <p class="text-danger">{{ $errors->first('password')}}</p>
                                @endif
                                <input type="password" name="password_confirmation" class="form-control my-4 py-2" placeholder="Confirm Password" required/>
